...
//            cell the centre is in after j ticks of another walk, taken as the goal, must get a bound <= j
...
//   jobs     (when the jobs folder has jobs: EEAT_HOME or src/jobs) the bound along the end of every job's best and
//            original run is at most the ticks the run really takes; 213 (the remake job): from OC's run (original.eetas)
//            the K = 29 search finds 2.35, checked with C.evaluate and C.judge
// usage: node test/endgame.js [--only=bound|exhaust|masks|jobs] [--seed=1]        Exit code 1 if any check fails.
const fs = require('fs');
...
function jobsSection() {
		...
		for (const run of ['best.eetas', 'original.eetas']) {
			...
			for (let t = 0; t < F; t++) {
				if (t >= F - 300) {
					...
					const lb = EG.lowerBound(B, sim, left + 1);
					checks++;
	...
	check(`${checks} states at the end of ${ids.length} jobs' runs: the bound is at most the ticks left`, bad === 0, bad ? firstBad : `${((Date.now() - t0) / 1000).toFixed(1)} s`);
