...
| `native/launch.h`, `test/gpulaunch.js` | bounded kernel launches: every launch of every eegpu command is split over index ranges into launches sized toward `--launch-ms` (default 50) from the measured time per item (start small, grow at most 1.5x, shrink at once after an overshoot), the phases in order, so the result is the same as one launch (a single launch past the display driver's 2 s watchdog reset the driver and once crashed a laptop whose GPU throttles to 1/6 of its clock); launches whose items cost differently are sized for their worst case: the explore / beam expand from the children really simulated, scaled to 18 per parent (twins are skipped: a user30s parent simulated 3 on average, and parents stay grouped by region), the search in candidate-ticks (every live candidate playing the whole segment, whatever the batch's size and twins); a 25 ms target made the 131072-wide beam about 17% slower (two runs each; its launches then hold too few parents to fill the throttled GPU), so it stays 50; the host thread spins for the first `--spin-ms` (1) of a launch and then sleeps on the launch's blocking-sync end event (`--wait=block`, the default; `--wait=spin` spins throughout): the same speed next to a job's GPU searcher and next to 15 busy CPU threads, with far less CPU time (an explore pass 0.47 s instead of 1.9 s, an 8 s search 0.38 s instead of 2.6 s: a spinning host thread took a whole core at above-normal priority); the done events report `maxLaunchMs` (host clock) and `maxKernelMs` (GPU clock), `kernelTotalMs`, `gapMs` (the GPU clock's time between one command's end and the next one's start) and `hostCpuMs` (the process's CPU time); a failed launch prints `{"error":...,"launchError":true}` and exits 6 (7: CUDA_ERROR_LAUNCH_TIMEOUT), with no retry inside eegpu (gpusearch.js waits 60 s and halves `--launch-ms`, and stops after 3 failures in a row; the editor stops its GPU strategies; the GPU benchmark is retried after 30 min). explore, beam, search and bench run at above-normal CPU priority (`--priority=normal` or `EEGPU_PRIORITY=normal`: off): their host thread starts each short launch, and next to busy CPU threads (the editor's CPU search) "every move" was 3-7x slower without it. Never kill eegpu while it runs a kernel (the driver resets the GPU): `--stopfile=<path>` ends it between two launches (its final line with end "stopped", exit 0) and `--parent=<pid>` does the same once that process has exited. Node kills the children it did not start detached the moment it exits (its job object; also when it is killed), so the GPU searcher, the editor and the guided search start eegpu with `detached: true` and `--parent`, and job stops (`jobs.js stopGpuSearcher`, before any tree kill) write the job's `gpu/stop` and kill the searcher alone. `--launch-items=N` (tests): at most N items per launch. `node test/gpulaunch.js [--lock=<gpulock.js>]`: short bench / explore / beam / search / trace runs, each launch under 3 x the target (6 x when nvidia-smi shows another process keeping the GPU busy before the test: the events around a kernel then count its time slices too; `--bound=<factor>`), explore / search / trace split into tiny launches (`--launch-items`) give exactly the ordinary run's layers, hits, edges file and hashes, and the explore with `--wait=spin` gives the same as with the default wait |
...
| `src/gpusearch.js` | the GPU searcher of a running job (grind `--gpu=1`): rounds of `eegpu search` split into invocations per family share (`--round=30`, `--sysShare=0.3` m1+del until one full pass over the current best, `--m2Share=0.2` m2, the rest one of pert / flip / sticky in turn; windows from cursors kept in `gpu/state.json` with the seed counter and per-family numbers), the edge library (`gpu/library.bin`, fingerprinted by eesim.js, the level blob and the coin mode), the union combine (`--union=40` most recent runs, `--siblings=1` other jobs of the same level; a library edge that does not replay is dropped; when the union's fastest combination is refused, the best alone with the library), verify, inbox; it searches its own judged run at once (until the grind refuses it); `gpu_status.json` (with `families`), `[gpu ...]` lines in grind.log (one line per round with every invocation's window, time, ticks and new shortcuts). `--tool=<file.js>` runs a stand-in for eegpu (tests without a GPU). Failures: a launch failure (eegpu's `launchError` line, exit 6 / 7) waits 60 s, then 120 s (`--launchWait`), halves `--launch-ms` and stops the searcher after 3 in a row or 5 in all; other failures (out of GPU memory, no context after a driver reset: no kernel ran) wait 15 s, doubled with every failure in a row up to 10 min (`--failWait`), logged for the first three and for a new error text; a run that works starts both over. `node test/gpusearch.js` (no GPU): both back-offs and the quit through the stop file, against a stand-in. `--every=1` (opt-in) adds "every move" rounds: `eegpu explore --rejoin=1` windows along the run (exact cells; every state equal to a later run state is a proven shortcut, re-checked with both hashes). Off by default: on full-size levels the exact windows explode (20-40 s per 40-60 ticks on a laptop GPU) and the search families find more per second |
| `tools/build-native.js`, `test/gpu.js` | build the native engine (`--exe`: eegpu.exe alone, one compile, no GPU kernels: enough for its CPU commands such as `eegpu prove`; a full build stamps its PTX with its kernel sources in `eegpu_ptx.json`, and `--exe` removes PTX files that no longer match, so a GPU command fails at its kernel load instead of running old kernels); the exactness proof (per-tick stateHash vs eesim.js; `--gpu` runs it on the GPU) |
| `src/eegfx.js` | EE graphics for the viewer, read at run time from the user's eeo-tas folder (`settings.json` `eegfxDir`, else `$EEO_TAS`, else `~/eeo-tas`): parses ItemManager.as / ItemId.as into a sprite map (block id -> sheet, 16 px frame, y, ItemLayer, shadow; the BlockSprites; morphable blocks; NPCs, smiley, death animation) and Player.as (`player`: the fire aura, the fly flame, the effect icons over the head, the zombie face), cached in `<data>/eegfx.json`. **Never commit EE images or derived sprite data**: the page loads the PNGs from eeo-tas through the server. The page's `gx*` functions follow World.as's draw rules. `node src/eegfx.js [dir]`, `node src/eegfx.js coverage <level.eelvl>...` |
| `src/editor.js`, `src/app/editor.html`, `test/editor.js` | the level editor (`/editor`): the editor's level JSON <-> `.eelvl` (`eelvl.js writeEelvl` / `readEelvl`), block info, checks (start, trophy, open way incl. portals and deaths that move the ball to another spawn; the physics check's "No way up" and "only through a death" notes), the route search: `eegpu explore - --finish=1` ("every move": exhaustive, one state per cell, the first finish = the fastest at its cell grain; a pass ladder (`passCells`, `passSeconds`, `nextPass`): it starts coarse (pass -1; coarse passes coarsen positions only, speeds stay at pass 0's 1/16 px/tick), a pass gets a share of the time (max(20 s, left / 3)) while a coarser pass is untried, goes coarser after a full table or a used-up share, finer after it ran out of states or found a route; with a route of T ticks every pass runs `--depth=T-1` (only faster routes) until the time is up; the "ran out of situations" verdict only from pass 0 or finer with `"overflow":0` in explore's `done` event, and it is evidence, not proof; salts: merged cells lose pixel-exact routes depending on which state stands for a cell, so with no next pass and time left the pass that ran through its depth runs again with the next `--salt` (another merged graph), the finest pass looping salts in one process (`--salts`, `{"ev":"try"}` events), several side by side (`--lanes=auto --lanesMax=8 --lanesStart=<the last run's>`: lane k = salt + k with cells of its own, exactly what that salt alone explores; the tool doubles the lanes while the tries per second rise by 10%, halves them when a batch fills the cell table, which is twice as large with lanes where the free memory allows; an explore that finds too little GPU memory when it allocates, e.g. the beams took it after its check, halves the table down to 2^24 cells with a `warn` line instead of failing); once every move ran out of situations at a fine grain with nothing cut and no route, the GPU beams stop so the tries get the GPU; the 40x25 shaft level's 251-tick run-up route comes from salt 8; near-miss refinement (`explore --refine=1`, on by default, the salt tries then one at a time: no lanes): a try that ran out hands its near misses to the next one (per unreached tile next to reached ones, the state that came nearest; every situation on their paths gets 4x finer x / vx cells), the user's 50x50 pixel-exact level 2.9 s, shaft 2.5 s; a refined try running past max(`REFINE_S` 10 s, 6x the plain try before it) ends its pass as full; the probe (`PROBE_S`): every move starts at the finest pass, and keeps it when the first try runs through within 15 s, else the ladder runs from the coarse end; the relay (strategy `relay`, "from the nearest attempt", GPU): once an attempt has gone `RELAY_MIN_TICKS` (100), `eegpu explore --prefix=<the nearest attempt's inputs, 60 (150, 400) ticks short of its end>` with a fresh table and 1/4 px/tick speed cells (4 px cells where no gravity pulls, e.g. dots, else 2 px): exhaustive from there, so a long route chains past one table and the beams' traps (a run-up detour, a corner the physics check thinks it can cut); a clearly nearer attempt restarts it at once; the dot ring (a 919-tick route) 33-37 s instead of never; a beam whose own nearest attempt has not improved for `BEAM_STALL_MS` (10 s) gives the GPU to the others, one still closing in keeps it; a GPU tool silent for `STALL_S` (20 s) after loading is stopped (stop file, then the kill)) next to `eegpu beam --goal=1` (with a guide line also a second beam `--guide --guideWeight=4 --goalWeight=4`; the beam's selection runs on the GPU: dedupe, score histogram, picks in score order with a per-bucket cap) and the CPU strategy `src/goexplore.js` (`cpu: true`; N - 1 worker threads, capped by the benchmark's fastest thread count and by half the threads while a job's optimizer runs; its depth limit is not cut by the beams' width; its routes bound the explore's next pass like any route, faster routes found elsewhere reach it on stdin as `depth D`; halted once every GPU strategy has ended with a route known, unless one of them failed); without an NVIDIA GPU / the native engine / a level eegpu supports, `start()` runs the CPU strategy alone (`S.cpuOnly` = the note), every route replayed in the JS engine; state in `<data>/editor/` (`solve.json`, written at most every 0.5 s during a search and at once at its start, stop and end: a write per event froze the server; `level.eelvl`, `route.eetas`); of each chunk of a tool's output only the last progress line is handled; Stop ends a search at once also while its physics / tool check runs (`searchGen`). The page copies the viewer's `gx*` drawing functions. `node test/editor.js [--gpu]` |
| `native/prove.h`, `src/prove.js`, `test/prove.js` | the proof of Find a route: `eegpu prove <level.bin> [--reach=<RCH3>] [--seconds=30] [--maxCells=6000000]` (CPU only, one thread: it never loads the NVIDIA driver, so it also runs without a GPU), a sound abstract interpretation of the engine's tick for levels of plain solids (not ice), air-like blocks (air, spawn, crown, coins, checkpoint) and the trophy, world gravity 1, a start with no effects and not inside a block (anything else: `"verdict":"unsupported"` with `why`). Abstract states: vy exact (on the fall orbit after a collision or the jump orbit), px / py / vx as boxes inside 1 px x 1 px x 1 px/tick cells, two run-up bounds (px - D(vx) >= olo, px + D(-vx) <= oLhi; D = the engine's run-up distance for a speed) that tie speed to position; the movement loop modelled exactly (partial steps, 1 px steps, the one big step for a left / up move from a whole pixel, failed steps retried while the other axis keeps the loop going, grounded, the jump, the x auto-align), tile coverage with the engine's exact rounding boundary, boxes cut back to free space after every tick, a swept-free fast path. The fixpoint over the 6 inputs holds every reachable state: no box with its centre in a trophy tile = `"impossible"` (a proof); `"reached"` = nothing proven; `"limit"` = out of time / cells. `--reach`: pieces the reach field cuts off are dropped (sound: the field is a proof; trophy tiles never dropped; about 6x faster on user50), the explanation then covers the ways the field leaves open (`pruned` > 0); only the field of this very level: the RCH3 header's level fingerprint (`gpu.blobFp`, FNV-1a 64 of the level blob, written by the editor's reach worker: `reachFileBytes(field, levelFp)`) must match the blob's, and a field that cuts off the start is not this level's either (both: a `warning` line, the proof without the field; another level's field with the same walls and the trophy moved once made user50 "impossible"). The done line's `explain`: the highest top of the box, the reachable centre tile nearest a trophy and its distance, the fastest speeds right / left, the start tile. Also `--check=<cases>` (the transfer against engine samples), `--noStop --states=<file>` (containment of a route), `--map=`, `--trace=x,y`. The editor (`src/prove.js`): once the reach field finds a way (finite start cost, physics mode), the proof runs next to the searches (30 s, at most the search's time; `prove.bin` + the reach file; killed `PV.WATCHDOG_S` after its budget without an answer; `EEAT_PROOF=0`: none, as in test/editor.js outside its prove section), cached in `<data>/editor/prove_<level hash>_<fingerprint>.json` (prove.h, eesim.js, eelvl.js, reach.js, gpu.js and the tool's build; the newest 8; a time limit is not kept, a memory limit is); a proof caps the search at `NO_WAY_UP_S` (10 s) of search time (a check: a route found anyway is a mistake in the proof, `model_miss.json` with `by: 'prover'`), and without a route the verdict is `S.impossible = {by: 'prover'}` with "No route (proven): ..." (`PV.message`: the highest the ball's top gets, the closest tile, the fastest run toward the trophy); a search whose strategies end first waits for the proof unless a route is known or every strategy failed; the proof (a CPU process) neither keeps the CPU search running once every GPU strategy has ended with a route (`cpuDone`) nor a job's GPU searcher paused (the busy file); `S.proof` = {state, verdict, sec, cells, pruned, reach, explain, cached, checkS}. user50 (263-tick route): "reached" in 1.6 s with the field (8.6 s without, on a loaded i7-11800H); its left run-up 2 tiles shorter: proven in 1.0 s (6.5 s without). `node test/prove.js [--only=scope,check,rooms,contain,reach,random] [--cases=12000] [--rooms=10] [--mutants]`: the scope, the transfer against the engine (random boxes near walls, floors, corners, ledges), rooms with known answers (test/reach.js's rooms, run-up gaps, user50 and variants, the shaft level), known routes contained tick by tick (also with `--reach`), the reach prune (same answers, fewer boxes; another level's file refused), random rooms against a concrete search; `--mutants` builds 6 broken copies (zig from tools/.cache) and the check must catch at least 5 |
| `src/goexplore.js` | the CPU route search of Find a route (Go-Explore): an archive with one cell per (tile, on ground, sign of vx, vy class, jump count, gravity queue, discrete state: coins and which, keys, switches, effects, checkpoint, gates, portal draws), each keeping its earliest state; a heap picks the lowest reach cost + 2 sqrt(picks); 8 random runs of 40 ticks per pick (an input kept with p 0.85); finer cells (4 levels, down to 1/16 px and 1/128 px/tick) in the 3 x 3 tiles around a cell picked a 6th time when the lowest reach cost has not improved for 200 picks; states the reach field rules out end a run. Snapshots only for picked cells, within a budget (`--mem`; evicted ones are rebuilt by replaying from the parent or the start). Worker threads with seeds `--seed`, `--seed + 1`, ...; one worker with a tick budget (`--maxTicks`) is exactly reproducible. Prints the editor's JSON events (`start`, `progress`, `closest`, `result` kind `finish` after `C.evaluate`, `done`); `--first=1`, `--depth=`, `--out=`, `--stdin=1` (`depth D`, `stop`; the end of stdin stops it too), `--prune=0` (the reach field rules nothing out: the editor's check of a level the field calls impossible) |
| `src/reach.js`, `test/reach.js` | the reach field (v3) of Find a route: a physics-aware cost to the trophy in fifths of a tile per abstract state (tile, type, level): R rising (the centre's apex in half rows above the tile's top edge: q -1 = lower half only, 0 = upper half, >= 1 = into the row above), F falling (the fall potential: the free-fall distance whose speed the ball has, plus the px to the tile's bottom edge, in rows), L falling in the lower half (no jump: it is never at standing height), C rising in a field (dots / side arrows, climbables, water, mud / lava, up arrows: the upward speed at the row's top edge with the push left in the row, 1/8 px/tick), XR left a field sideways in this row (no pumping). The physics is one forward move function (`fwd`) plus same-tile edges (the jump, the apex, a field's stop / bounce / turn, boosts) and portals / death respawns; a backward label-setting search in cost buckets inverts it per pair of tile profiles. Half blocks: the collision follows the stored rotation (presents 1101-1105 too), the current tile (gravity, kills) the engine's rule (presents always the tile above). Deaths (with a checkpoint or 2+ spawn points: the respawn at the checkpoint touched last, else the next spawn of EE's rotation, 255 and 1582 #0): an edge from every tile the ball can die in (a killing current tile; anywhere with a timed killer: curse, zombie, poison, lava) to every respawn tile at `DEATH_COST` (8192 fifths: finite, so no proof is lost, but behind every real way; the searches drop dead balls, and the editor says when the model's only way is a death). Every table is the engine's own arithmetic (rises from a speed and gravity queue, the free-fall orbit, the jump 63.42 px; fields' pushes and top speeds checked against the engine), and every rule errs toward reachable: -1 (cut off) is a proof: the explore prunes those states (`--prune=1`, after its finish test) and the editor calls a cut-off start impossible. The lookup (`fifthsAt`, the same doubles as native/beam.h `reachFifths`) uses the gravity queue, the ceiling and slipperiness; a rising ball in a row next to fields is both R and XR (the higher cost: each alone is a valid description). Levels with jump / fly / speed / low-gravity / multijump / gravity effects or another world gravity: walking distance (walk mode, never a proof). `reachField(level, {check, explain, goals, maxCost})` (Bellman self-check; when the start is cut off, the highest row it reaches; explore.js --hunt's time-to-go field), `costAt(field, sim)` (tiles), `scoreAt` (the beam's score: the cost blended between the 4 tile centres around the ball, the ball looked up at each, ways through a death left out next to a real one; = beam.h `reachScore`, which shares the lookups' position-independent parts: the rise, the fall), `writeReachFile` (RCH3 for eegpu `--reach=`; `eegpu info` says `"reach":3`, an older tool is refused with "the search tool is older than the app: rebuild it"), `shareField`. The editor builds it in a worker thread (also for the page's checks, which only read the cache and ask again while it builds), cached in `<data>/editor/reach_<level hash>_v3_<model fingerprint>.bin`/`.json` (the fingerprint of reach.js, eesim.js and eelvl.js: a changed model never reads an old verdict). `node test/reach.js [--only=A..G] [--gpu] [--tool=] [--jobs=]`: A the tables against the engine (and the lookup's rise against the engine's apex), B rooms with engine answers (ledges, dot steps, strip-k, up-pump, the limits of single rules, the user's 50x50 shaft level ranked; every state from which a small engine search reached the trophy finite) and routed rooms (deaths as a way to move, presents at every rotation, a down boost in the jump's gravity queue), C every state of every job's runs and the known editor routes reachable, D a step fuzz (cut off at t implies cut off at t + 1, through deaths and respawns), E the self-check and the goals options, F the JS lookup = `eegpu reachtest` (the fifths and the beam's score; host; `--gpu` the GPU too; a host build with `-DRF_CHECK` checks every table index), G build times (200x200: 300 ms target) |
...
| `src/skips.js` | skip search: route skips found on purpose. Windows: contact pass-bys (the run passes within 96 px of a spot where it lands, or a wall stops it, 40-900 ticks later; slack = the gap - distance / 8) and loops (`loops.js`, 48 and 96 px), the `--scan` best; entrances: every move (BFS by tick, one state per cell: 1 px, 1/4 px/tick, ground, jumps, gravity queue, context; the earliest arrival; in a tube of 3 tiles around the run) for `--span` ticks from the run `--lead0` ticks before the pass-by: a state that touches a run contact spot (the same floor within 24 px, the same wall) `--minLead`+ ticks before the run does (or comes within 24 of a run state that far ahead); routes: for the `--top` windows by their best entrance, `--K` entrances each (the corners of each tick's group: min / max of x + vx, vx, x; contacts first), every move from each on its own until `--cap2` ticks in a 2-tile tube around the run after the contact; exact rejoins and tails (re-anchored at landings / walls like mutate --anchor) -> edges, DP, judged (`<out>.edges.json`); `--targets=<runs>` (the grind passes the job's 4 newest earlier bests): a state equal to one of theirs, or a tail along one that meets it, makes a whole run through it, judged; worker threads, `--seconds`. Forgotten Veil best_11257 (the 88-tick "mini 10" skip nothing else rediscovered): -83 after 48 s on 4 threads, -94 after mutate (`tools/rediscover.js --only=skips`); fragile (from a run differing a few ticks there only with its earlier bests as targets: -49, -53 after a splice); not Octorage's loop skip (explore on the loop window) nor Infinity Pain's long route changes. `test/explore.js`: a hand-made pass-by room |
| `src/explore.js`, `test/explore.js` | Go-Explore route explorer for a window (`--from --join --until --exact=1`); `--tails=1` keeps every exact rejoin as an edge (DP, `<out>.edges.json`); `--hunt=1` (off by default) is guided skip hunting with the same edges: a time-to-go field of the reference per context (reach.js `goals`, built before the search: 2-3 s of CPU per context on a 200x200-400x200 level, shared by the workers), one archive cell per (tile, ground, vx sign, vy class, jumps, context), picks half by most lead and half by novelty, tails from cells ahead of the reference (it finds local skips `--tails` misses and misses long detours a wide `--tails` window finds: a complement; numbers in the header); `--ticks=N` = an equal tick budget per worker. `--clockblind=1` (time-door levels, with `--tails=1`): rejoins by `stateHashClockBlind`, written as edges for `phase.js --edges=<out>.edges.json`, which replays each one plain or with the clock re-synced in the idle start `node test/explore.js`: `--hunt` and `--tails` on a hand-made room (exact edges, a judged run, deterministic) |
...
| GET | `/api/jobs/:id/trajectory?which=best` | the run for the viewer, simulated with the job's level JSON (rng_script, start_mode): `ticks`, `complete`, `runTicks`, `timerStart`, `version`, base64 little-endian arrays `x`, `y` (Int32, px x 16, top-left of the box, index = tick 0..ticks), `run` (Int32 run timer), `flags` (Uint8: 1 dead, 2 on ground, bits 2-4 gravity 0 down 1 up 2 left 3 right 4 none), `inputs` (masks), `events` ([tick, kind, ...]: coin/blue_coin x y, portal fx fy tx ty, death, respawn, jump, key color x y, key_expired, switch kind id on, checkpoint/crown/complete x y), `doors` ({"id:number": [solid at 0, toggle ticks...]}), `coinsTaken0`, `clock0` (the level clock at tick 0, for time doors), `effects` (the ball's effects, exact from the engine; null when it never has one: `on` [tick, bits, tick, bits, ...] = the bits from that tick on (1 protection, 2 curse, 4 zombie, 8 poison, 16 fire, 32 fly, 64 thrusting, 128 low gravity, 256 god mode, 512 jump, 1024 speed, 2048 multijump, 4096 gravity, 8192 team; bits 14-16 while thrusting: the side eeo-tas draws the fly flame on, 1 below 2 above 3 left 4 right 0 none), `values` [[tick, name, ...]] when a value changes while its effect is on (`jump` / `speed` / `gravity` / `team` value, `multijump` maxJumps jumpCount, `curse` / `zombie` / `poison` / `fire` killTick ticks: the first dead tick and the timer's length, -1 0 = no timer), `thrust` [tick, thrust x 100, ...] only where it is not the rule's (the last tick's while thrusting, else the last tick's - 1, not below 0)); for `best` also `align` (Int32: the original's first tick at the same point, per best tick) and `original` {runTicks, version}. `which=original`: the uploaded TAS |
...
| GET | `/api/eegfx` | EE graphics (src/eegfx.js): `{available, dir, source, why, version, sheets, sizes, blocks: {id: [sheet, frame, y, layer, shadow]}, sprites, rot, npcs, smiley, player, death, numbers, ids}` (`player`: Player.as's fire aura, fly flame and effect icons as [sheet, first frame, w, h, frames], the zombie face's smiley frame); `available: false` with `why` when no eeo-tas folder is found |
