...
| `src/explore.js` | Go-Explore route explorer for a window (`--from --join --until --exact=1`); `--tails=1` keeps every exact rejoin as an edge (DP, `<out>.edges.json`); `--hunt=1` (off by default) is guided skip hunting with the same edges: a time-to-go field of the reference (reach.js `goals`), one archive cell per (tile, ground, vx sign, vy class, jumps, context), picks half by most lead and half by novelty, tails from cells ahead of the reference (it finds local skips `--tails` misses and misses long detours a wide `--tails` window finds: a complement; numbers in the header); `--ticks=N` = an equal tick budget per worker |
