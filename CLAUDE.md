...
| `tools/build-native.js`, `test/gpu.js` | build the native engine (`--exe`: eegpu.exe alone, one compile, no GPU kernels: enough for its CPU commands such as `eegpu prove`); the exactness proof (per-tick stateHash vs eesim.js; `--gpu` runs it on the GPU) |
