...
- **The grind cycle** (src/grind.js): rounds of about 10 minutes (`--roundMin`): mutate loop, the exact endgame
  ...
  exploring windows (every other one with `explore --hunt=1`, guided skip hunting; `--hunt=0` off; first up to 5 loop windows (60% of the round; Infinity Pain: 4 of 7 loop windows found time): stretches where the run comes back within 48 px (then, all tried, 96 px) of where it was with nothing
