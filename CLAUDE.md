...
| `src/reach.js`, `test/reach.js` | the reach field of Find a route: a physics-aware cost to the trophy per (tile, rise budget), the budget in units of 8 px above the tile's middle, every source rounded up so the model never rules out a real route. Sources measured with the engine: a jump from a floor or a ledge edge (63.42 px: 8 units; 9 with half blocks), the landing-tick jump under no-jump tiles after a 15-tile fall (83 px), dots / side arrows by column height (+1 unit per tile climbed inside), water, ladders, mud / lava, up arrows (also a trampoline for a falling ball), up boosts and portal exits (unlimited). Going up costs 1 unit per half tile in gravity tiles; sideways keeps the budget; falling ends it; walls and deadly tiles block; portals lead to their exits. -1 = unreachable is a proof: the explore prunes those states (`--prune=1`) and the editor calls such a trophy impossible. Levels with jump / fly / low-gravity / multijump / gravity effects or another world gravity fall back to walking distance. `reachField(level, {check})` (Bellman self-check), `costAt`, `writeReachFile` (RCH2; eegpu `--reach=`, native/beam.h `ReachField` / `reachAt`). `node test/reach.js`: rooms with engine-checked answers, the self-check on random levels, and every state of every job's original and best run must be reachable |
