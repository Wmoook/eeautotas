...
| `src/reach.js` | the reach field of Find a route: a physics-aware cost to the trophy per (tile, rise budget) (a jump from a floor or a ledge edge rises ~4 rows, dots / side arrows / climbables / liquids 1 + h/2, up arrows 1 + h, an up boost or a portal exit unlimited; sideways moves keep the budget, falling ends it; walls and deadly tiles block; portals lead to their exits). Optimistic on purpose, so -1 = unreachable is a proof: the explore prunes those states (`--prune=1`) and the editor calls such a trophy impossible. Levels with jump / fly / low-gravity / multijump / gravity effects or another world gravity fall back to walking distance. `reachField(level, {check})` (Bellman self-check), `costAt`, `writeReachFile` (eegpu `--reach=`, native/beam.h `ReachField` / `reachAt`) |
